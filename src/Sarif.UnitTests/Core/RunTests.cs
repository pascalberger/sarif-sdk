...
namespace Microsoft.CodeAnalysis.Sarif
{
    public class RunTests
    {
        ...
        public void Run_RetrievesExistingFileDataObject()
        {
            ...
            ArtifactLocation artifactionLocation = BuildDefaultFileLocation();
            artifactionLocation.Index.Should().Be(-1);

            // Retrieve existing file location. Our input file location should have its
            // fileIndex property set as well.
            RetrieveFileIndexAndValidate(run, artifactionLocation, expectedFileIndex: 1);

            // Repeat look-up with bad file index value. This should succeed and reset
            // the fileIndex to the appropriate value.
            artifactionLocation = BuildDefaultFileLocation();
            artifactionLocation.Index = Int32.MaxValue;
            RetrieveFileIndexAndValidate(run, artifactionLocation, expectedFileIndex: 1);
        ...
        private void RetrieveFileIndexAndValidate(Run run, ArtifactLocation artifactionLocation, int expectedFileIndex)
        {
            int fileIndex = run.GetFileIndex(artifactionLocation, addToFilesTableIfNotPresent: false);
            artifactionLocation.Index.Should().Be(fileIndex);
        ...
        private Run BuildDefaultRunObject()
        {
            var run = new Run()
            {
                Artifacts = new[]
                {
                    new Artifact
                    {
                        // This unused artifactionLocation exists simply to move testing
