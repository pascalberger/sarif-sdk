    ...
    public partial class Artifact : ISarifNode
    {
            ...
            IFileSystem fileSystem = null)
        {
            ...
            try
            {
                ...
                {
                    artifact.Contents = GetEncodedArtifactContents(fileSystem, filePath, mimeType, encoding);
                }
        ...
        private static ArtifactContent GetEncodedArtifactContents(IFileSystem fileSystem, string filePath, string mimeType, Encoding inputFileEncoding)
        {
            var artifactContent = new ArtifactContent();
            byte[] ArtifactContents = fileSystem.ReadAllBytes(filePath);

            if (SarifWriters.MimeType.IsBinaryMimeType(mimeType) || inputFileEncoding == null)
            {
                artifactContent.Binary = Convert.ToBase64String(ArtifactContents);
            }
            else
            {
                artifactContent.Text = inputFileEncoding.GetString(ArtifactContents);
            }
