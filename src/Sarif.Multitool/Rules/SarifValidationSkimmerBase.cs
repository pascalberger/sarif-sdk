...
namespace Microsoft.CodeAnalysis.Sarif.Multitool.Rules
{
    public abstract class SarifValidationSkimmerBase : Skimmer<SarifValidationContext>
    {
        ...
        protected virtual void Analyze(ArtifactChange artifactChange, string fileChangePointer)
        {
        }

        protected virtual void Analyze(ArtifactLocation artifactionLocation, string fileLocationPointer)
        {
        }

        protected virtual void Analyze(Artifact artifact, string filePointer)
        {
        }
        ...
        private void Visit(Artifact artifact, string filePointer)
        {
            Analyze(artifact, filePointer);

            if (artifact.Location != null)
            {
                Visit(artifact.Location, filePointer.AtProperty(SarifPropertyName.ArtifactLocation));
            }
        }

        private void Visit(ArtifactLocation artifactionLocation, string fileLocationPointer)
        {
            Analyze(artifactionLocation, fileLocationPointer);
        }
        ...
        private void Visit(ArtifactChange artifactChange, string fileChangePointer)
        {
            Analyze(artifactChange, fileChangePointer);

            if (artifactChange.ArtifactLocation != null)
            {
                Visit(artifactChange.ArtifactLocation, fileChangePointer.AtProperty(SarifPropertyName.ArtifactLocation));
            }
        }
