...
namespace Microsoft.CodeAnalysis.Sarif.Visitors
{
    public class InsertOptionalDataVisitor : SarifRewritingVisitor
    {
        ...
        {
            ArtifactLocation artifactionLocation = node.Location;
            if (artifactionLocation != null && _run.OriginalUriBaseIds != null)
            {
                bool workToDo = false;
                bool overwriteExistingData = _dataToInsert.HasFlag(OptionallyEmittedData.OverwriteExistingData);
                ...
                if (workToDo)
                {
                    if (artifactionLocation.TryReconstructAbsoluteUri(_run.OriginalUriBaseIds, out Uri uri))
                    {
